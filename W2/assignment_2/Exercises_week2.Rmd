...
Next, we move on with the analysis of `duration` variable.

<h3>EDA of `duration`</h3>

The `duration` variable is the last contact's duration expressed in seconds. Since this variable is continuous, we plot it as a histogram with bin size of 60, because that represents 60 seconds, which is a minute.
...
To identify outliers of the data, we will plot it as a boxplo groupped by `deposit`.
...
The above boxplot identifies outliers beyond `r 329 + 1.5 * (329 - 104)` seconds. Any observation beyond that duration is deemed an outlier. Next, to deal with the outliers, we replace the outliers with NA, impute data, and finally replot it as a density graph groupped by `deposit`.
...
ggplot( data = bank) + 
  geom_density( aes( x = duration, fill = deposit ), alpha = 0.3)
```
It is clear from the above graph, that calls that last less than approximately 220 seconds has no success in convincing the customer to subscribe for the deposit. Further, we identify the tendency that the longer the phone call lasts, the better the subscriber rate.

Next, we move on with the `campaign` variable. 

<h3>EDA of `campaign`</h3>

The `campaign` variable stands for the number of contacts performed during the campaign.

First we plot it on a bar chart, whereby we identify the distribution's positive skewness. That is, most of the bank customers were only contacted a few times. Furthermore we identify some outliers based on the width of the x-axis. To further deal with the outliers, we first plot the data on a boxplot identify the outliers, then mutate and impute the data and finally plot it as a density graph.
...
ggplot( data = bank) + 
  geom_density( aes( x = campaign, fill = deposit ), alpha = 0.3)
...
The above plot does not indicate strong graphical evidence of predictive importance of `campaign`.

Next, we move on with `pday`.

<h3>EDA of `pday`</h3>

`pday` stands for the number of days passed since the customer was contacted. It may take on the value `-1`, that means that the customer was not contacted previously, and the current campaign contact is the first.

First we plot the frequency of each day passed as a bar chart.
...
We not only see that the values along the x-axis spreads out on a wide range but also, that there are a large quantity of `-1` observations, referring that the large amount of customers were not contacted previously.

To see the real distribution, we investigate a subset of customers who were contacted previously. That is, we filter out those with `pdays = -1`, and re-plot the graph as a bar chart and boxplot.
...
The boxplot identifies some outliers, that we first replace by NA values and then impute them, and finally depict it as a line chart.
...
ggplot( data = bank2) + 
  geom_density( aes( x = pdays), alpha = 0.3)
```

From the imputed data we see, that the majority of customers who were contacted previously, were contacted again for a follow up contact between 100 and 350 days.

To see the success rate of each of the `pdays`, we plot it as a density plot groupped by `deposit`.
...
Based on the above standardised graph, we see no strong graphical evidence of the predictive importantce of `pdays`. Therefore, we do not expect the data mining algorithm to include `pdays` in the model.

Next, we move on with `previous`.
...
We see that the majority of the customers were not contacted previously. This observation coincides with the findings in `pdays` , since that also shown, that that the majority of the customers were not contacted.
...
From the above plot we read, that until the first 15 contacts, on average the success rate increase. That is, the more call is directed towards the customer, the higher the chance the customer subscribes. (revise!)

Although, the finding above is not strong but considerable, we can expect the data mining algorithm to include it in the model.

Finally, we move on with `poutcome`.

<h3>EDA of `poutcome`</h3>

`poutcome` represents the outcome of the previous campaign.
...
```{r}
  ggplot( data = bank) +
    ...
    scale_fill_manual( values = c( "red", "blue" ) )
```
We see that the unknown category has the highest count. This category is likely to represent the customers that were not contacted previously, therefore, is coincides with data of `pdays` and `previous`.
...
We interpret the above graph as that those previously subscribed for the deposit are more likely to subscribe again. The above plot indicate strong graphical evidence of the predictive importance of `poutcome`. The data mining algorithm will include `poutcome` in the model.
