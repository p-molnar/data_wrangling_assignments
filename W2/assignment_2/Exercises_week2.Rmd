...
<details class="answer_container">
...
**Statistical datatype of variables**
  
* `state`         : categorical - nominal
* `area.code`     : categorical - nominal
* `account.length`: numerical - discrete
* `voice.plan`    : categorical - binary
* `voice.messages`: numerical - discrete
* `intl.plan`     : categorical - binary
* `intl.mins`     : numerical - continuous
* `intl.calls`    : numerical - discrete
* `intl.charge`   : numerical - continuous
* `day.mins`      : numerical - continuous
* `day.calls`     : numerical - discrete
* `day.charge`    : numerical - continuous
* `eve.mins`      : numerical - continuous
* `eve.calls`     : numerical - discrete
* `eve.charge`    : numerical - continuous
* `night.mins`    : numerical - continuous
* `night.calls`   : numerical - discrete
* `night.charge`  : numerical - continuous
* `customer.calls`: numerical - discrete
...
<details>
  ...
  473 customers have an international plan.
</details>
...
<details>
  <summary>Answer</summary>
The first plot depicts a bar chart of `intl.plan`, with `churn` overlay. The chart shows that approximately 500 customers have international plan, while the rest 4500 did not choose that plan. Although the proportions of `churn` is not clearly readable, we can somewhat see that approximately half of the customers who have international plan tend to churn. This rate is much smaller for customers who do not have international plan.

The right chart is the standardised version of the bar chart on the left. Hence its y-axis ranges between 0 and 1, and shows the proportions of churners in each `intl.plan` category. From that we can clearly read that approximately 42% of internatinoal plan subscribers go to a different company. Further, we see that this proportion is only approximately 10% for those not holding an international plan. That is, for international plan subscribers churn rate is four times higher than for non subscribers.
</details>
...
On the left is a bar chart for `voice.plan`, with `churn` overlay. We see that approximately 1300 customers have voice mail plan, and 3700 do not. From the graph the churn rate is not clear, therefore its standardised equivalent is needed.

On the right, the standardised bar chart is found, and shows the approximately 8% churns from voice mail plan holders. This rate is roughly twice as much for those not holding a voice mail plan, and is approximately 16%.

Given the above interpretation, it is advised to include `voice.plan` for our data warngling software, since it may explain the overall 14.14% churn rate.
...
The histogram above shows the number of times the customer service was called, with `churn` overlay. Since the histogram is not clear for interpretation, we turn to the standardised histogram to be able to read the proportions.

We read from the standardised histogram, that churn rate at least quadruples as the number of phone calls to the service calls exceed 3 calls.

The standardised and non-standardised plots must be used together, since the last bar may be misleading. That is, if we did not know the shape and size of the original histogram, we would conclude that 100% customers who has called the customer service 9 times has churned. However, this only affects just a few observations. 
...
The above boxplots depict the locality and spread and skewness of `day.mins` grouped by `churn` categories.

We see that `day.mins` is negatively skewed for those who churned.

The median of churners is at 215 minutes, which means that the 50% of all observations are below 215 mintues and the other 50% is above that value. Since we established that the data is skewed negatively, it means that the the upper 50% of the data spread out on a smaller range, and thereby its density must increase. This finding let us conclude that as a customer's day minutes increase above 215, its tendency to churn also incrasing. That is, customers who consume more than 215 minutes inclined to churn more than otherwise.

The boxplot for non-churners depicts an approximate symmetry, i.e., the data is cut in half at approximately 175 minutes. This boxplot report some outliers as well below 45 and above 310 minutes.
...
The boxplot on the left represents churners, and the right non-churners. Both non- and churners are positively skewed, and have their medians at approximately 4 calls.

Both plots report outliers above approximately 10 calls. The boxplot, however, does not indicate strong evidence of the predictive importance of international calls.
...
From the above correlation matrix we see that there are 4 problematic variable that shows perfect correlation with another variable.

Namely, `night.charge` is perfectly linearly correlated with `night.mins`, which does not come as surprise, since the former is a function of `night.mins`. The same holds true for `eve.mins` and `eve.charge`, `day.mins` and `day.charge`, `intl.mins` and `intl.charge`.
