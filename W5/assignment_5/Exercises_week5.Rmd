...
* The root node represents 38782 observations (100%). This node is a decision node, and splits on `marital.status` into 2 groups, married and unmarried.
  ...
  * Those of married represent 18239 observations, 47.02% of the whole dataset. This node is a decision node, and splits on `education.num`.
...
In conclusion, the model with n=100 shows a slightly better prediction metrics than the model with n=10. Since the margin is little to non-significant, if the larger n value comes at the cost of computational complexity, and hence computational time, the model with the smaller n is preferred. However, if the larger n does not cause any computational difficulties, the larger n is more optimal.
...
* The Random forest and C50 algorithms have equally low Mean Squared Error, that is, they bears with the lower error on average in prediction than CART
* The random forest and C50 algorithms have equal accuracy, and they are both higher than that of CART's
* The random forest algorithm has the highest rate of sensitivity
* the C50 algorithm has the highest specificity rate
* C50 has the highest AUC, therefore it is deemed to be a superior model to others.

In conclusion, Although the C50 and Random forest algorithm produce similar results, because of the higher AUC value, C50 is preferred over the other 2 models.
...
```{r}
( mse_random_forest = mse( predict_cart, actual_test ) )
( mse_random_forest = mse( predict_C50, actual_test ) )
( mse_random_forest = mse( predict_random_forest, actual_test ) )
( mse_random_forest = mse( predict_knn, actual_test ) )
```
...
* In terms of MSE, the random forest algorithm produces the least erroneous prediction
* the most accurate algorithm is the random forest, and hence also the one with the smallest misspecification rate
* in terms of sensitivity, again the random forest came out as superior to other models
* in terms of specificity KNN bears with the highest rate, however, the other models are not far behind either
* in terms of AUC, the random forest algorithm encloses the largest area under the curve

All things above considered, the random forest is deemed the best prediction model out of the 4 competing models.
