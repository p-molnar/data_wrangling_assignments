...
**Interpretation:** We are $95\%$ confident that the true population mean is included in the range $[1.534181, 1.606619]$.
...
**Interpretation:** We are $95\%$ confident that the true population mean is included in the range $[0.9084033, 2.0915967]$.
...
**Statistical conclusion:** Given that p-value = $7.015 * 10^{-3}\%$ < $\alpha = 1\%$, there is sufficient evidence found to reject the $H_0$, and infer that population mean of the number of customer service calls is greater than $1.5$.

**p-value:** $7.015 * 10^{-5} = 7.015 * 10^{-3}\%$
...
**Interpretation:** We are $95\%$ confident that the true population proportion is included in the range $[0.1319201, 0.1514362]$.
...
**Statistical conclusion:** Given significance level of $\alpha = 0.01$ , and p-value = $60.45\%$ > $\alpha= 1\%$, there is insufficient evidence found to reject the $H_0$ in favour for $H_a$, and we infer that population proportion of churners greater than or equal to $0.14$.
...
Based on the statistical output above, we see a higher mean number of customer service calls for churners (2.254597), than for non-churners (1.457722). That is, there is a positive statistical relation between churning and number of customer service calls.
...
**p-value:** $0.3523 = 35.23\%$

**Statistical conclusion:** Given significance level of $\alpha = 0.05$ , and p-value = $35.23\%$ > $\alpha = 5\%$, there is insufficient evidence found to reject the $H_0$ in favour for $H_a$, and we infer that the mean number of day minutes is not greater than 180 minutes.
...
**p-value:** $1.04 * 10^{-5}  = 1.04 * 10^{-3}\%$

**Statistical conclusion:** Given significance level of $\alpha = 0.05$ , and p-value = $1.04 * 10^{-3}\%$ < $\frac{\alpha}{2} = 2.5\%$, there is sufficient evidence found to reject the $H_0$ in favour for $H_a$, and we infer that the mean international charges differs for churners and non-churners.


**Conclusion:** There is statistical relationship between international charges and churn, given that the mean international charges are significantly different (with $\alpha=0.05$) for churners and non-churners.
...
\bigg\{
\begin{matrix}
          H_0:  \pi_{primary, yes}  = \pi_{secondary, yes} = \pi_{tertiary, yes} = \pi_{unknown, yes} \\
          H_a: \text{at least one of the claims under } H_0 \text{ is wrong}
\end{matrix}
\]

**p-value:** $0.001625  = 0.1625\%$

**Statistical conclusion:** Given significance level of $\alpha = 0.05$ , and p-value = $0.1625\%$ < $\frac{\alpha}{2} = 2.5\%$, there is sufficient evidence found to reject the $H_0$ in favour for $H_a$, and we infer that there is statistical relationship between `education` and `deposit`.
