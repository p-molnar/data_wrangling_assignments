---
title: '<center>Data Wrangling Main Project<center>'
subtitle: '<center>Predicting employee turnover rate in the field of Data Science<center>'
...
Employee turnover is a general labour market phenomenon whereby employees rotate between organisations, roles, and occupations, and fluctuating between the states of employment and unemployment ([Abassi et al., 2000](#ref_1)).

Turnover may be involuntary, in the case of of poor employee performance, or voluntary, where the employee chooses to part ways. Regardless of the reason and the type of turnover, it is extremely costly for organisations.

The replacement of an employee is a time-consuming and costly procedure that involves writing out the new position, advertising, pre-selecting promising candidates, carrying out a series of interviews, and eventually making the hiring decision. Hence employee turnover is usually considered the most general and frustrating problems that organisations have long been facing.

According to the estimates of the Bureau of Labour Statistics, on average an employee replacement costs approximately \$13,996 per employee. This amount, is estimated to be 42% higher for the Information Technology (IT) sector, and on average reaches \$20,000 per employee. As the economy expands, more organisations opens their doors for new candidates, and the larger supply of workplaces induces an increase in voluntary employee rotation.

Since, turnover is not an accountable cost, nor is it typically defined in the budget, organisations tend to underestimate the real cost associated with losing even a single employee. No only the explicit replacement fees can set back companies by tens of thousands of dollars, but the implicit costs, such loss in productivity, workplace safety issues, and morale damage add to the bill as well.

The best measure to reduce employee turnover is an improved selection process that assesses applicants' turnover risk, and engagement early in the hiring process. As such, it can lead to the reduction of turnover, and by that to a safer, more productive and profitable work place. ([O'Connell & Kung, 2007](#ref_2))

## Dataset description

The selected dataset, ["HR Analytics: Job Change of Data Scientists"](https://www.kaggle.com/datasets/arashnic/hr-analytics-job-change-of-data-scientists), is on a Big Data and Data Science specialised company, that seeks to extend their team with promising candidates.  The company offers a series of training courses, that anyone can sign-up for, and upon the completion, the candidate can decide whether to stay at the company or not.
...
* What type of candidates should the company consider hiring to minimise the risk of turnover?
...
The above outputs shows, that data is imported as a data.frame object, and that it consists of `r nrow(data)` rows and `r ncol(data)` variables. The `r ncol(data)` variables includes the target variable, called `target`.

The [dataset](https://www.kaggle.com/datasets/arashnic/hr-analytics-job-change-of-data-scientists) provider delivers the following description to the features:
...
Since the above summary does not allow to gain the most in-depth understanding on the features, in the coming sections we further explore them.

## Understanding the variables

The structure above reports that `r names(Filter(is.character, data))` are imported as character variables, and `r names(Filter(is.numeric, data))` as numeric ones.
...
# select character type columns
categorical_data = data[ , c(categorical_vars) ]         

# report the unique values of each feature
...
The `company_type` variable holds `r length(unique(categorical_data $ company_type))` unique values, and stands for the type of the company the candidate currently is working for, if any.
...
The `last_new_job` variable holds `r length(unique(categorical_data $ last_new_job))` unique values, and stands for the difference in years between the previous job and current job. The variable can also be interpreted as the number of years the candidate is at his/her current workplace.
...
```{r}
# get column index position of 'enrollee_id'
col_idx = match( "enrollee_id", names(data) )

# remove enrollee_id column from dataset
data = data[ , -col_idx]
```
...
From the above output we see that dataset contains `r sum( is.na(data) )` missing observations, which regards in total `r count(missing_values[missing_values["pct_miss"] > 0, ]["variable"])` variables. These are: `r missing_values[missing_values["pct_miss"] > 0, ] $ variable`.
...
# replace observations to numeric, to be able to interpret correlation matrix
...
```
The matrix reports a problematic pair of variable, `company_size` and `company_type`. We see that there is a high positive correlation ($\rho_{copmany\_size, company\_type} = 0.8$) between the missing values of  `company_size` and `company_type`. Meaning, that if `company_size` is missing, then there is a high chance that `company_type` will be missing as well, and this holds true vice versa. This may be the case, because `company_type` is usually a function of `company_size`. For example, if we know that a company has only a few employees, then there is a high chance that the form of the company is Early stage start-up. Furthermore, if we only know the type of the company, we cannot make a rough classification of its size, hence the high correlation. In this case, the high correlation is justified, but we do not find it problematic to drop them.
...
### Dealing with the severely impacted variables

When data is burdened to such large extent by missing values, we cannot use imputation because it may come at the expense of data reliability and distorts it. Therefore, the two options are to either drop the missing observations, or replace them with a new "Unknown" value. Although the above variables are impacted by 30%, it is still not as severe to drop rows with missing observations. 

Therefore, we proceed with the former approach, and replace missing observations with "Unknown" value.

```{r}
# select variables with more than 5% missing values
...
### Dealing with the lightly impacted variables
When data contains only a small fraction of missing values, we can utilise different imputation methods. In this case, we will perform an imputation, which replaces the missing values with random ones, proportional to the categories' records.
...
Next, we report a summary on the imputed variables.
```{r}
# create imputed variable collection
imputed_vars = c("education_level", "last_new_job", "enrolled_university", "experience")

# get summary on imputed variables
...
Based on the above output, we can state that the dataset is free from NA observations.

## Converting the variables

At this stage, the data no longer contains missing values, and are clean, however, each of the variables are either numeric or character type.

Below the table summarises the appropriate data type of the variables that in this section we will convert each of them.
...
```{r}
# get summary on modified data
summary(data)

# get structure of modified data
str(data)
```

# Exploratory Data Analysis

This section aims to identify useful predictors of the target variable, `is_looking_for_job`. We use graphs, plots, and tables to uncover important relationships that could indicate important areas for further investigation. Here we start with the categorical variables, and then continue with the numerical ones.
...
# plot a bar chart with percentages drawn on the graph
...
addmargins( table (data $ is_looking_for_job) )

# get proportion table
prop.table( table(data $ is_looking_for_job) )
```
...
With the `city` variable, we attempt to understand which cities are the ones where the most candidates came from and the ones that were the most successful in terms employment. First, we plot the `city` as a normal and then as a standardised bar plot.

```{r, fig.show = "hold", out.width = "50%", fig.align = 'default' }
# plot bar chart with overlay
ggplot( data = data ) + 
...
# plot standardised bar chart with overlay
ggplot( data = data ) + 
...
Since, there are `r length(unique(categorical_data $ city))` unique values plotted on the x-axis, it does not allow for clear identification of the cities. Therefore, we create a temporary data frame out of a standard contingency table for `city` variable, and add a few new columns to it; `candidate_count`, which is the sum of the candidates from a given city, `application_rate`, which is rate of application per city compared to the overall dataset, and finally, `turnover_rate`, that shows the percentage of candidates that wants to change job in the given city.
...
First we order the data in a decreasing order based on the application rate, so that we see from which city most applicants sign up from, and report the top 5 results.

```{r}
head(city_df[ order(city_df $ application_rate, decreasing = TRUE), ])
```

We see that approximately `r sum(head(city_df[ order(city_df $ application_rate, decreasing = TRUE), ]) $ application_rate) * 100`% of the applicant came from the above 5 cities. We also report, that the turnover rate does not seem to follow a pattern against the turnover rate, but they are rather random.
...
```{r}
# select specific rows of data frame
cand_count_more_30 = city_df[city_df $ candidate_count > 30, ]

# get top 5 results in decreasing order of data frame where there are more than 30 candidates
head(cand_count_more_30[ order(cand_count_more_30 $ turnover_rate, decreasing = TRUE), ])
```

The above output shows, that turnover rate again does not seem to follow a pattern against application rate, therefore it is advised to test whether there is significant correlation between the application rate, and the turnover rate of the given city.
...
\bigg\{
\begin{matrix}
          H_0:  \rho_{application\_rate, turnover\_rate} = 0 \\
          H_a:  \rho_{application\_rate, turnover\_rate} \neq 0
\end{matrix}
\]

**Test results:**
```{r}
# carry out two-sided correlation hypothesis test
...
**Test results:**
```{r}
# carry out chi-squared hypothesis test 
...
The company should consider hiring male candidates, as they have the lowest tendency of leaving the company after the training.

**What are the main characteristics of candidates that the company loses? **

The company is unable to establish proper engagement with those of unknown gender category.
...
# plot data as a standardised bar chart with overlay
ggplot( data = data ) + 
...
This result may be explained by the fact that those without relevant former experience, lose interest in the field after the training, and hence look for job elsewhere.

**What type of candidates should the company consider hiring to minimise the risk of turnover? **

The company is advised to track candidates carefully based on their previous relevant experience. The company should consider hiring candidates with relevant experience they tend to leave the company approximately 10% less likely than those without relevant experience.

**What are the main characteristics of candidates that the company loses? **

The company loses out on candidates approximately 35% of times amongst those who lack experience in the field of Data Science.
...
The contingency table shows that the majority of candidates (14087) are/were not enrolled to any higher education, while those who took either full or part time courses at the university are in minority (5071).
...
The above plots suggest the company to pay attention to those candidates who were/are enrolled at a full time course, since their turnover rate significantly stands out from the `enrolled_university` categories. That is, the company needs to focus on hiring those with no enrolment at university.


**What are the main characteristics of candidates that the company loses? **

The company cannot establish proper engagement with those with full time enrolment, and hence loses them in 37.5% of the time.
...
Given that `education_level` is an ordinal variable, the bar plot draws a symmetry for the data. That is, most of the candidates in the dataset hold a graduate degree, while towards the sides we find the rarer cases, which are of those with Masters and High School qualifications. Towards the tails, we find the rarest observations, i.e., those with significantly lower and higher qualifications, such as Primary School diploma and Phd.

To see the proportion of turnover for each of the education levels, we read the standardised plot, and report that it kept its approximate symmetric shape for turnover rate. Furthermore, we see that those with a graduate degree tend to change job the most likely, by a little bit more than 25%, while the rest of the categories change job with a likeliness of less than 25%. Those with High School degrees want to change approximately with the same frequency as that of with Masters degree, which is approximately 20%. Similarly, those with Primary School degree wants to change job approximately as frequently as Phd degree holders, that is approximately 13%.
...
**What type of candidates should the company consider hiring to minimise the risk of turnover? **
The above plots highlight that the company needs to target those with Primary school certificate, or High school Diploma, or Masters degree or Phd. However, on the field of Data Science the higher qualification is generally more favoured, therefore the company should go with candidates specifically with Masters or Phd degrees.


**What are the main characteristics of candidates that the company loses? **
The company loses standard graduates in a bit more than 25% of the times.
...
To minimise turnover risk, the company should hire those with Arts, Humanities, and Unknown degree.


**What are the main characteristics of candidates that the company loses? **

The company loses Business, STEM, and Other degree holder candidates the most,  as these candidates tend to look for new opportunities the most amongst the discipline categories. 
...
The above plots show, a multiple local modes to the data. The first we identify at around experience of 4 years, the second at around experience of 10 years and lastly at 15 years. The mode of the data however, is ">20", so we report that those with more than 20 years of experience represent the majority of the dataset.
...
The above identified relationship suggests the company to target the senior candidates in order to minimise turnover risk as their likeliness to be loyal to the company is significantly higher, than those of juniors.


**What are the main characteristics of candidates that the company loses? **

The company unequivocally loses those candidates with less than 8 years of experience.
...
The above plot shows that candidates with unknown company size represent the majority of the data. Next to that, in majority as well those who work for companies of size "50-99" and "100-499" and large organisation of size more than 9999 employees.
...
Both the data, and conclusion above suggests the company not to take candidates from companies of unknown size, instead, 

**What type of candidates should the company consider hiring to minimise the risk of turnover? **

The company should focus on hiring attempt candidates from companies of headcount less than 10, "50-99", "100-499", "1000-4999", in order to minimise turnover risk.

**What are the main characteristics of candidates that the company loses? **

The company loses out the most on those candidates with unknown headcount and those with "10-49".
...
Similarly to the previous variable, `company_size`, here the plots suggest the company to hire candidates funded start-ups, and private limited firms.


**What are the main characteristics of candidates that the company loses? **

the company cannot engage with candidates from unknown company types, and early stage start-ups and companies of type "other".
...
The above plot also suggests the company to look for candidates who stays at their current company as long as possible. To minimise risk, this means at least 4 years at the current workplace.

**What are the main characteristics of candidates that the company loses? **
The company cannot engage with candidates of no former employment, and those with less than 2 years at the current workplace.
...
The graphical and numerical evidence suggest the company to candidates with more than 200 hours spent on the course.

**What are the main characteristics of candidates that the company loses? **

Those finishing the training less than 200 hours tend to leave the company. Therefore, the company loses these candidates in the largest percent.
...
# get summary on `city_development_index`
summary(data $ city_development_index)

# convert data from impute to numeric
...
The data suggest the company to hire candidates from cities with at least 0.75 CDI, in order to minimise turnover risk.

**What are the main characteristics of candidates that the company loses? **

The company loses engagement with those from lower CDI cities (below CDI of 0.7).
...
* `major_discipline`: The variable has predictive capabilities. The company need to focus on groups with major discipline of Art, Humanities and those with unknown discipline, as they are the most loyal to the company after the course completion.
...
* `training_hours`: The variable has strong predictive importance. The company needs to focus on candidates with at least 200 hours spent on the training.

**What are the key predictors of employee turnover? **

Based on EDA, we find that the key predictors of turnover are the following variables: `city_development_index`, `gender`, `relevant_experience`, `enrolled_university`, `education_level`, `major_discipline`, `experience`, `company_size`, `company_type`, `last_new_job`, `training_hours`.
...
# create train set and test set out of data_sets
train_set = data_sets $ part1
test_set  = data_sets $ part2

# get target variable of test set
...
Given significance level of $\alpha = 0.05$ , and p-value = $52.54\%$ > $\alpha = 5\%$, there is insufficient evidence found to reject the $H_0$ in favour for $H_a$, and we infer that the proportion of those who want to leave the company are not different between the training set and the test set. This indicates, that the partition for the target variable, `is_looking_for_job` is valid.
...
Based on the training dataset, we apply different type of Machine Learning algorithms, by using above predictors in our model. We use the following formula:

```{r}
# declare formula for modelling 
...
To find out the optimal value of `k` for the k-nearest neighbour algorithm we plot the error rate for each `k` for $k \in [1, 20]$.

We achieve this by using the `kNN.plot()` function, and set a seed of for the sake of reproducibility.

```{r fig.align = 'center'}
# get kNN error rate plot 
kNN.plot( formula, train = train_set, test = test_set, transform = "minmax", k.max = 20, set.seed = 5 )
```

The plot shows that the minimum value of error rate is obtained for `k = 19`. Since the smaller values of error rate indicates better predictions, $k=19$ is the optimal value.
...
Given that the variables in this dataset do not have the same scale, one variable may overwhelm the contribution of another's. For this reason, we use a min-max normalisation and transfer the predictors.
...
The above plot shows a single decision node, which is the root node. The root node represents 100% of the observations, out of which 75% classify as not looking for a job, and the rest 25% as a candidate who wants to change after the completion of the training. The root node splits at `city_development_index`, based on whether a city has a CDI at least 0.62 or not.

We read that 82% of the candidates are from cities with City Development Index >= 0.62, while the rest 18% are from lower CDI classified cities.

82% of those from the higher CDI cities classify as not wanting to change job, while the rest 18% does want to change job after the training.
...
Based on the kNN algorithm, the only predictor is City Development Index.
...
```{r}
# get summary on C5.0 prediction
summary( tree_C50 )
```

**What are the key predictors of employee turnover? **

According to C5.0, the following predictors bear with the highest level of usage: `city_development_index`, `company_size`, `major_discipline`, `company_type`, `experience`, `education_level`.
...
We see that `n` obtains its minimum approximately around `ntree=25`. After this value we see that the error rate is roughly constant.

Below we plot a dot-chart for each variable to see how each variable contributes to the homogeneity of the nodes and leaves in the the above reported random forest.  The higher the value of mean decrease Gini score, the higher the importance of the variable in the model.

```{r}
# get Random Forest mean decrease Gini score plot
varImpPlot( random_forest )
```

**What are the key predictors of employee turnover? **

Based on the Random Forest algorithm, the most significant predictors are `city_development_index`, `training_hours`, `experience`, `company_size`, `last_new_job` and `company_type`. These are the features the company needs to dedicate attention for when hiring candidates based on the random forest algorithm.
...
```{r}
# get summary on logistic regression
summary( logreg )
```
# Model Evaluation

Above we dealt with 5 different classification methods: k-nearest Neighbour Algorithm, CART, C5.0, Random Forest and Logistic Regression. To decide which of the algorithms are the most accurate for prediction, we evaluate them by the ROC and AUC. The AUC is the best method across the learned evaluation metrics, because it incorporates the confusion matrix at multiple splits. The highest AUC indicates the highest combined specificity and sensitivity of the model, and by the that its accuracy.

```{r fig.align='center'}
# get prediction for variables
...
prob_knn = kNN( formula, train = train_set, test = test_set, transform = "minmax", k = 19, type = "prob" )[ , 1 ]
prob_logreg  = predict( logreg,  test_set, type = "response" )

# get ROC curve for variables
...
# plot ROC curves, and get AUC values for variables
ggroc( list( roc_knn, roc_cart, roc_C50, roc_random_forest, roc_logreg ), size = 0.8 ) + 
    theme_minimal() + ggtitle( "ROC plots with AUC for 5 outcomes") +
...
The above plot shows that the CART algorithm performs the worst of all 5 models with its AUC of 0.343, and the Random Forest the best with AUC of 0.803. Therefore, we can conclude that the company should use the Random Forest algorithm for prediction and rely on the following variables when making hiring decisions: `city_development_index`, `training_hours`, `experience`, `company_size`, `last_new_job` and `company_type`.

# Conclusion

In the first section, [Business Understanding Stage](#business-understanding-stage), we explored the general problem employee turnover, and introduced the objectives of the dataset at hand.

We then moved on with the [Data Understanding Stage](#data-understanding-stage) section and obtained more insight into the data, by exploring the unique values that the categorical variables took on, and made remarks about the problematic parts of the data.

The third section, [Data cleaning and preprocessing](#data-cleaning-and-preprocessing), solved the issues about the dataset, e.g., resolved inconsistencies, fixed up overlapping data, and converted character variables into the variables' statistically appropriate data type.

After then, in the forth section, [Exploratory Data Analysis](#exploratory-data-analysis) we analysed each variables by means of Exploratory Data Analysis and sorted the variables into 2 bins; useful for predictions and not useful. Also, we unveiled important relationships between the given variables and the target variable. In EDA, we also provided an answer to the type of candidates the company needs to consider in order to minimise the risk of turnover, as well as identified those that the company could not engage with.

Next,in the [Data Preparation](#data-preparation), we started the modelling phase of the paper, and partitioned the data into training and test set. 

Once the partitions were proved to be valid, the [Modeling section](#modeling---classification) started. In this section, we used five different models to classify variables, and answered the main question of what are the key predictors based on the different classification methods.

Finally, we evaluated the competing models based based on their AUC value, and found that the Random Forest is the best out of all 5 models.

# References

<span id="ref_1">1.</span> [Abassi SM, Hollman KW (2000). "Turnover: the real bottom line", Public Personnel Management, 2 (3) :333-342.](http://ithuteng.ub.bw/bitstream/handle/10311/1154/Ongori_AJBM_2007.pdf?sequence=1&isAllowed=y)

<span id="ref_2">2.</span> [O'Connell, M., & Kung, M. C. (2007). The Cost of Employee Turnover. Industrial Management, 49(1).](https://www.researchgate.net/publication/211392097_The_Cost_of_Employee_Turnover)
