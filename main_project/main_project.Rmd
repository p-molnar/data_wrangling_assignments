...
```{r}
(tbl = as.data.frame.matrix((table(data $ city, data $ is_looking_for_job))))

names(tbl) = c("looking for job", "not looking for job")
tbl["candidate count"] = tbl $`looking for job` + tbl $`not looking for job`
tbl["application rate"] = round(tbl $`candidate count` / length(data $city), 4)
tbl["turnover rate"] = round(tbl $`looking for job` / tbl $`candidate count`, 4)
```
...
```{r}
head(tbl[ order(tbl $`application rate`, decreasing = TRUE), ])
```
...
```{r}
cand_count_more_30 = tbl[tbl $`candidate count` > 20, ]
...
**Test results:**
```{r}
cor.test( x = tbl $`application rate`, y = tbl $`turnover rate`, 
...
When `training_hours` is plotted as a standardised graph with the target variable overlay, we see that those who spend more than 200 hours with the training tend to look for a job less likely than those who completes the training under 200 hours. Based on this, we see strong enough graphical indication of the predictive importance of `training_hours`, and hence we would expect the data mining algorithm to incorporate that into the model.

Next, we plot the `training_hours` against `is_looking_for_job` on a boxplot. We report that the boxplot identifies a large quantity of outliers for both `is_looking_for_job` groups beyond 175 hours. Calculating with 8 hours of training a day, we find that 175 hours comes down to 21 days, which is a month. Since a month of training, or even more than that is normal, we will not treat the above identified outliers as invalid, therefore we leave them intact.

the above result suggest the company to extend the training course beyond 200 hours, since that decreases turnover rate.
...
The above box plots indicate that for the 2 subgroups, the distribution is significantly different. In the case of those who are not looking for a job after the training, we see a few outliers towards the lower end of the [city development index](https://en.wikipedia.org/wiki/City_development_index). In less developed cities, the opportunity to study Data Science, or even develop an affinity towards it is rare. Mainly because of the poorer condition does not allow to specialise towards this field. Therefore if a candidate comes from so undeveloped cities, and passes the training course, is indeed an outlier. Therefore the data first needs to be imputed, by values drawn randomly from an underlying distribution.

```{r}
q1 = boxplot(data $ city_development_index) $ stats[2, ]
q3 = boxplot(data $ city_development_index) $ stats[4, ]
iqr = q3 - q1
...
The above histogram shows that `city_development_index` is negatively skewed. Meaning, that the majority of the candidates come from cities with higher city development index. When we standardise the plot, we see a negative relationship between `city_development_index` and the target variable. That is, the higher the city development of the city the candidate comes from, the lower the likeliness he/she will be open to new opportunities after the training.

Meaning, there is a clear graphical evidence of the predictive importance of `city_development_index` of the target variable, therefore, we expect the data mining algorithm to incorporate into the prediction model.


## Conclusion of EDA {.unlisted .unnumbered}

Based on the above analysis, we can conclude the followings:
* 
