...
* predicted "no" for 908 people, out of which 834 was correctly predicted, while 74 was incorreclty.
...
Currently, MSE is measured in squared percentages, an dto interpret it, we need to take the root of MSE to interpret in the same unit as the original data is in.
...
Between the two competing models, the lower the RMSE, the better the forecast, hence kNN with k=10 is the more optimal choice for prediction.
...
Between competing models, one is preferred over that other that produces low error rate and and does not overfit neither underfit the data. Since the differences in error rates on the above graph is small (vertical distance), an optimal k-value, with decent level of fitting, can be anywhere between 18 and 22. 
</details>
...
* Out of the 1040 predictions, the model correctly predicted the outcome 888 + 3 = 891 times.
* Out of the 1040 predictions, the model incorrectly predicted the outcome 152 times
...
* The restricted model bears with approximately 6.7% better error rate, than the complete model. That is, on average the restricted model is a better predictor, and is preferred over the complete model.
...
Based on the above summary, we can conclude that the restricted model is a better predictor. Furthermore, we can conclude that in the complete model `state`, `area.code`, `intl.calls`, `intl.charge`, `day.mins`, `day.charge`, `eve.calls`, `eve.charge`, `night.mins`, `night.calls`, `night.charge` in this comnbination do not bear with significant predictive importance.
