...
The average price a diamond sells in the `diamonds` dataset is $`r  round(mean_price, digits = 3)`.

```{r}
min_price <- min(diamonds["price"])
```

The cheapest diamond of the `diamonds` dataset was sold for $`r  min_price`.

```{r}
max_price <- max(diamonds["price"])
```
The most expensive a diamond of the `diamonds` dataset was sold for $`r  max_price`.
...
The histogram above shows 2 potential outliers; one at `r min_x`mm and another at `r max_x`mm. The outlier at `r min_x`mm is likely to be an entry error, since a diamond's width cannot be 0mm.

The other end of the data at `r max_x`mm requires further investigation, but since the histogram follows an approximate normal distribution centered around the mean of `r round(mean_x, digits = 3)`, it seems that the value `r max_x`mm is not an outlier.

If we plot the the `x` variable on the x-axis against `price` on the y-axis, we see that the larger values towards `r max_x` cost more, which is a likely case. Therefore, `r max_x` is not deemed to be an outlier.
...
The below code replaces the outliers with NA, and plots the new data.
...
The takeaway from the scatterplot is a positive correlation between the `x` variable and `price`. That is, the greater the width of a diamond, the more valuable it is, and hence sells for a higher price.
...
The histogram above shows 2 potential outliers; one at `r min_z`mm and another at `r max_z`mm. The outlier at `r min_z` is likely to be an entry error, since a diamond's depth cannot be 0mm.

Not only looking at the extreme values of `z`, but also the width of the histogram gives away, that the there is likely to be an outlier. It happens to be the case that there is a data entry at `r max_z`mm which requires further investigation to justify whether it is an outlier or not.

To do so, we plot the `z` variable against `price`.
...
It seems unusual that a diamond with a depth over 3cm does not cost a fortune, so it may be the case, that when recording the data a decimal point was placed at the wrong place.

The below code replaces the outliers with NA values, and plots the data again.

```{r}
diamonds_2 = mutate( diamonds_2, z = ifelse( z == 0 | z > 10, NA, z) )
