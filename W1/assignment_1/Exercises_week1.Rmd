...
Considering the validity of scenario number 2; the data documentation does not make any indication that the dataset records unrealised losses as 0 capital losses.
The validity of scenario number 3 is also questionable. Firstly, markets tend to move in a way that it impacts one's capital balance, and hence it is unlikely that one manages to have a portfolio that ends up being in 0 capital loss. Second, individuals invest for the hope of closing investments with profit, instead of at 0 gain and loss. Conclusively, the only plausible scenario is number 1. That is, those who did not invest did not incur any losses and hence were recorded 0 as capital loss.

For the non-zero observations, that is a subset of people who invest, we see an approximate bell curved distribution centered around mean losses of \$1873. The middle 50% of the distribution consists of average performing investors who  reported capital losses between \$1672 and \$1977.


```{r}
adult_removed_zero_closs = mutate( adult, capital.loss = ifelse( capital.loss ==  0, NA, capital.loss ) ) 

ggplot( adult_removed_zero_closs ) +
  geom_histogram( aes( x = capital.loss ), bins = 30, color = "blue", fill = "lightblue" )
summary(adult_removed_zero_closs $ capital.loss)
```
...
Use flag variables like `has.invested`.?
