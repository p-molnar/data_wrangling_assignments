...
The summary output for `capital.loss` shows that the data is positively skewed, because the median < mean. Furthermore, we can report that extreme values from the summary; the smallest value that `capital.loss` takes on is 0, while the largest one is 4365. Lastly, we can conclude from the summary output that more than 75% of the data consists of 0 observations, based on the first, second (median) and thirds quartiles. Furthermore, on average the an indivual of the data set has suffered from a capital loss of $87.94.

The second plot is a boxplot for `capital.loss` and it shows an immense positive skewness at value 0. Not only the distribution of the data can be read from the plot but also the extreme values reported above; minimum of 0 and maximum of approximately 4300.

The last graph is a histogram, that collects observation frequencies into bins of width 30. We see a large quantity of zero observations, and some other observations in the middle of the grap around 2000. Although we cannot see the maximum value from the graph, we can just suspect that it is somewhere around 4000, since this is how far the x-axis of the plot strecthes. This alludes to fact that the right extreme is around 4000.
...
There are two peculiarities that immediately outstand from the graph. First, the large count of 0 capital losses. Second, the approximately symmetric distribution of the non-zero losses.
...
The validity of point number 2 also lies on shaky grounds as well. Firstly, markets tend to have some level of movements that has a direct impact over investors capital balance. Hence, it is highly unlikely that one manages a portfolio in a way that it ends up being in exactly null-saldo, i.e., in 0 capital gain and loss. Second, individuals invest with the primary purpose of making profits, instead of closing off the investment at 0 gain and loss. Therefore, we can conclude that the large count of 0 observations are unlikely to be because of the investors sold the investment at the same price it was purchased for.

Consequently, point number 1 and 2 are week enough assumption to be relied on, and so discarded.

Considering point number 3, it is testable how the values for `capital.loss` and `capital.gain`
```{r}
...
Since we interpreted the shape of the distribution as an approximate bell-curve, we would replace the outliers with values taken from a random normal distribution centered around the same mean as the original data. This way we can guarantee the measure of location and the spread to preserve its original shape.
...
We see from the above summary output, that the first and the third quartile are at 0, meaning that at least 75% of `capital.gain` is 0. Similarly to exercise 4.4, we replace the 0 observations with NA values, so to limit down the observations to individuals who invested and realised capital gains. After then, we report a summary of the mutated data, and plot it as a boxplot. 
...
To have an idea of the mutated data's distribution we replot a histogram.
...
The distribution of `capital.gain` does not show strong enough similarities to normality or similarity, therefore we do not apply the same imputation method as in exercise 4.4. Since `capital.gain` is a numerical attribute, we replace the above qualified outliers with the mean value of the data, and replot it.
...
```{r}
adult_5["has.invested"] = ifelse(adult["capital.gain" == 0] & adult["capital.loss" == 0], TRUE, FALSE)
```
