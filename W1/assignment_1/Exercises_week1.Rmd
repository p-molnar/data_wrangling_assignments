...
```{r echo=FALSE}
min_x = min(diamonds $ x)
max_x = max(diamonds $ x)
```

The limits of the x-axis does not have a wide limit — the observations spread out withing a range of [`r min_x`, `r max_x`]. By looking at the two extreme values of `x`, minimum at `r min_x`mm, and maximum at `r max_x`mm, we can already rule out 0mm, since a diamond's width cannot be 0mm. It requires further investigation to understand how the data were recorded, and whether 0mm is a measurement error, or if it is a codified indication of a missing value.

On the other end of the range, `r max_x`mm, also requires further investigation to be deemed as an outlier or not. For that we calculate the relevant boxplot values for `x`. For that we report the quartiles of `x` and utilise the below formulas to calculate $Q_4$.

```{r}
quantile(diamonds $ x, prob=c(.25,.5,.75))

q1 = unname(quantile(diamonds $ x, prob=c(.25)))
q3 = unname(quantile(diamonds $ x, prob=c(.75)))

iqr = q3 - q1
q4 = q3 + 1.5 * iqr
```

From the above output and with the help of the below formulas, we can calculate the parameters of the boxplot.
...
Q_4 = Q_3 + 1.5 * IQR
$$
In our case the relevant metrics is the boxplot's $Q_4$ value, which results to `r q4`, which is smaller than `r max_x`. Given that `r max_x` is located outside of $Q_4$, it is numerically so distant from the rest of the data that it can be deemed an outlier.
...
The below code replaces the unusual values with missing values, and plots the new data.

```{r}
diamonds_3 = mutate( diamonds_2, x = ifelse( x == 0 | x > q4, NA, x) )

ggplot( data = diamonds_3, mapping = aes( x = x, y = price ) ) + 
  geom_point( colour = 'blue' )
```

Below we report a summary of variable `x` with the new missing values present in the dataset, whereby we see that 32 values were replaced with missing values.

```{r}
summary(diamonds_3 $ z)
```

  <h3>Outliers in `z`</h3>
  
To see if variable `z` contains any outliers, first we are going to plot it as a histogram and look for anomalies, and observations that are far from the bulk of the data.
...
The observations along the x-axis are widely spread out within the range of [`r min_z`, `r max_z`]. By looking at the two extreme values of `z`, minimum at `r min_z`mm, and maximum at `r max_z`mm, we can already rule out 0mm, since a diamond's width cannot be 0mm. It, however, requires further investigation to understand how the data were recorded, and whether 0mm is a measurement error, or if it is a codified indication of a missing value.

On the other end of the range, `r max_z`mm, also requires further investigation to be deemed as an outlier or not. For that we calculate the relevant boxplot values for `z`. For that we report the quartiles of `z` and utilise the above used formulas to calculate $Q_4$.

```{r}
quantile(diamonds $ z, prob=c(.25,.5,.75))

q1 = unname(quantile(diamonds $ z, prob=c(.25)))
q3 = unname(quantile(diamonds $ z, prob=c(.75)))

iqr = q3 - q1
q4 = q3 + 1.5 * iqr
```

From the above output and by plugging in the values to the formula we can calculate the relevant statistics. $Q_4$ results to `r q4`, which is smaller than `r max_z`. Given that `r max_z` is located outside of $Q_4$, it is numerically so distant from the rest of the data that it can be deemed an outlier.

Below we plot variable `z` against `price`. 
...
```{r}
diamonds_4 = mutate( diamonds_3, z = ifelse( z == 0 | z > q4, NA, x) )

ggplot( data = diamonds_4, mapping = aes( x = z, y = price ) ) + 
  geom_point( colour = 'blue' )
```

Below we report a summary of variable `x` with the new missing values present in the dataset, whereby we see that 59 values were replaced with missing values.
...
<details class="answer_panel">
  <summary>Answer a</summary>
* nominal variables: `demogweight`, `marital.status`, `occupation`, `relationship`, `race`, `gender`, `native.country`
...
The above output shows, that "<=50K" is associated with integer value `1`, and ">50K" with `2`. Although in this format the data is already binary in a sense that it can only take 2 mutually exclusive values, but is not binary in its literal sense. That is, it is not within the boundaries of [0, 1]. If we were to take the mean of `income` now, it would result in a value 1 greater than the mean of a strictly binary variable. To work with binary values, we need to convert the factor records to values between 0 and 1. To achieve this, we took away 1 from each records.
...
Capital loss of 0 may be deemed as an outlier, however, it has relevant meaning. Capital loss of 0 means, that the individual either did not suffer any capital loss, or did not invest, and accordingly did not lose anything.
