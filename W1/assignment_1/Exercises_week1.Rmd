...
To make the unusual values along the x-axis more visible, we adjust the limits of y-axis.
...
We see the `r max_z`mm greatly deviates from the rest of the data. Such a diamond is over 3cm in depth, so we would like to see whether this observation is in line with its price to conclude whether it is an outlier or not. For that we plot `z` against `price` in a histogram.
...
There are two peculiarities about the above graph. One of them is the absorbent amount of 0 capital losses, and the other one is the symmetric distribution of those who suffered from non-zero losses.

Per the above definition capital loss of 0 may come down to either of the below scenarios:

1. The individual did not invest and hence could not incur any capital losses on the investment.
2. The individual made an investment, but did not sell the asset during the time window the data was recorded, and hence he/she had not suffer any realised loss, that is capital loss.
...
Unlike in the case of the `diamonds` dataset where 0mm was an invalid record, capital loss of 0 is a valid observation. Therefore, it is not advised to classify them as outliers, since they hold relevant information.
