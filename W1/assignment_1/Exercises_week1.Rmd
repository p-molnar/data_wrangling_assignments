...
There are two peculiarities about the above graph. One of them is the large count of 0 capital losses, and the other one is the approximately symmetric distribution of the non-zero losses. It is worth to note, that unlike in the case of the `diamonds` dataset where 0mm was an invalid record due to the characterisitcs of the variable, capital loss of 0 is a valid observation. 

Per the definition of capital loss, 0 observations may come down to either one of the below four scenarios:

1. The individual made an investment, but did not sell the asset during the time window the data was recorded, and hence he/she had not suffer any realised losses, that is capital loss.
2. The individual made an investment, and sold it at the exact same price the asset was originally purchased for, and hence incurred 0 capital loss.
3. The individual made an investment, but incurred capital gain.
4. The individual did not make an investment and hence could not incur any capital losses on the investment.

Considering the validity of each of the above scenarios, in point number 1 the data documentation does not make any indication that the dataset records unrealised losses as 0 capital losses. Therefore, we cannot justify with certainty that the large amount of zero capital loss observations is due to unrealised saldos.

The validity of point number 2 is also shaky. First, markets tend to have some level of volatility that impacts one's capital balance. Hence, it is highly unlikely that one manages a portfolio in a way that it ends up being in exactly 0 capital loss and gain at the same time. Second, individuals invest with the purpose of realising profits, instead of closing off the investment at 0 gain and loss. 

Consequently, scenario number 1 and 2 are week enough to be ruled out as an explanation for the large quantitiy of 0 observations.

Considering point number 3, it is testable if it is always the case that if one realises gain, then the capital loss is 0, and vice versa. To that end, we write the following code:
```{r}
# is there any observations, where both capital.gaint
# and capital.loss is greater than zero
any(ifelse(adult $ capital.loss > 0 & adult $ capital.gain > 0, TRUE, FALSE))
```

Given the above output, we can conclude, that `capital.loss` and `capital.gain` are mutually exclusive values. That is, if one makes an investment, the investor can either realises gain or loss, but not the two together. This realisation leads us to point number 4, which questions whether the individual made an investment at all. Above we argued about point number 2 being a shaky assumption, and hence we can say, that if `capital.loss` and `capital.gain` are both 0, the individual did not make an investment.

For the non-zero observations, that is a subset of people who incurred capital loss, we see an approximate bell curved distribution centered around mean losses of \$1873. The middle 50% of the distribution consists of individuals who reported capital losses between \$1672 and \$1977. The left tail of the histogram consists of investors losing between \$155 and \$1672, while the right tail of investors losing from \$1977 up to \$4356.

Given that in the current state of the data, `capital.loss` is burdened with a large quantity of zero observations, we replace them with NA values, so that `capital.loss` would only account for those investors who make losses on their investment.
...
Next, we depict the mutated data, as a boxplot, and report the quartiles, and whiskers.
...
We report that 46316 observations were replaced by NA. According to the

To preserve the 0 observations to some extent, we introduce a new flag variable `has.invested`, that indicates if the individual is an investor or not.
